...
test.beforeEach(async ({ page }) => {
  await page.addInitScript(
    ({ nowMs }) => {
      ...
      (window as Window & { Date: DateConstructor }).Date =
...
test("desktop default state", async ({ page }) => {
  ...
  await expect(page).toHaveScreenshot("home-clock-desktop-default.png", {
    fullPage: true,
  });
});
...
test("mobile default state", async ({ page }) => {
  ...
  await expect(page).toHaveScreenshot("home-clock-mobile-default.png", {
    fullPage: true,
  });
});
...
test("selection interaction state", async ({ page }) => {
  ...
  await expect(page).toHaveScreenshot(
    "home-clock-desktop-selection-change.png",
    { fullPage: true },
  );
});
...
test("empty state", async ({ page }) => {
  ...
  await expect(page).toHaveScreenshot("home-clock-desktop-empty-state.png", {
    fullPage: true,
  });
});
...
test("sky error state", async ({ page }) => {
  ...
  await expect(page).toHaveScreenshot("home-clock-desktop-error-state.png", {
    fullPage: true,
  });
});
...
test("command palette opens from trigger button", async ({ page }) => {
  ...
  await expect(page).toHaveScreenshot(
    "home-clock-desktop-command-palette-open.png",
    { fullPage: true },
  );
});
...
test("command palette opens from keyboard shortcut", async ({ page }) => {
  ...
  await expect(page).toHaveScreenshot(
    "home-clock-desktop-command-palette-hotkey-open.png",
    { fullPage: true },
  );
});

test("command palette keyboard navigation updates active option", async ({
  page,
}) => {
  await page.setViewportSize({ width: 1440, height: 1024 });
  await mockApi(page);
  ...
  page.on("console", (message) => {
    ...
    void (async () => {
      ...
      if (label !== "[react-mainline:event]") {
        return;
      }

      const payload = (await args[1]?.jsonValue()) as { type?: string } | null;
      if (payload?.type) {
        loggedEventTypes.push(payload.type);
      }
    })();
  });
  ...
  const searchInput = page.getByRole("searchbox", { name: "Search commands" });
  ...
  await expect
    .poll(() => loggedEventTypes.filter((type) => type === "NAV.NEXT").length)
    .toBe(1);
  await expect
    .poll(() => loggedEventTypes.filter((type) => type === "NAV.END").length)
    .toBe(1);
  await expect
    .poll(() => loggedEventTypes.filter((type) => type === "NAV.HOME").length)
    .toBe(1);
});

test("command palette emits console event logs for interactions", async ({
  page,
}) => {
  await page.setViewportSize({ width: 1440, height: 1024 });
  await mockApi(page);
  ...
  await expect
    .poll(() => loggedEventTypes.length, {
      message: "Expected command palette events to be logged to console.",
    })
    .toBeGreaterThanOrEqual(4);

  await expect
    .poll(() => loggedEventTypes.includes("PALETTE.TOGGLE"))
    .toBeTruthy();
  await expect
    .poll(() => loggedEventTypes.filter((type) => type === "NAV.NEXT").length)
    .toBe(1);
  await expect
    .poll(() => loggedEventTypes.filter((type) => type === "NAV.PREV").length)
    .toBe(1);
  await expect
    .poll(() => loggedEventTypes.includes("ITEM.ACTIVATE"))
    .toBeTruthy();
});
