...
function createClientBaselineEnvironment(
  timezone: string,
): SharedSkyEnvironment {
...
function fanOutProgressFromNoon(
  progress: number,
  minute: number,
  noonMinute: number,
): number {
  ...
  const distanceRatio = clamp(
    circularMinuteDistance(minute, noonMinute) / DEFAULT_NOON_MINUTE,
    0,
    1,
  );
...
function interpolateOrbitPoint(
  ...
  if (
    direction !== 0 &&
    Math.sign(deltaAngle) !== 0 &&
    Math.sign(deltaAngle) !== direction
  ) {
    const directedDelta =
...
function isPointWithinLayoutRect(
...
function inferLayoutCornerAttachment(
  layout: OrbitLabelLayout,
): OrbitCornerKey | null {
  ...
  const ratioY = pointRatioWithinRect(
    layout.spokeEndY,
    layout.y,
    layout.height,
  );
...
function hubFacingCornerForAnchor(
  anchorX: number,
  anchorY: number,
  centerX: number,
  centerY: number,
...
function shouldAllowCornerSwitchSweepOverride(
  ...
  const sourceDefaultCorner = hubFacingCornerForAnchor(
    source.anchorX,
    source.anchorY,
    centerX,
    centerY,
  );
  const targetDefaultCorner = hubFacingCornerForAnchor(
    target.anchorX,
    target.anchorY,
    centerX,
    centerY,
  ...
  return (
    sourceCorner !== sourceDefaultCorner && targetCorner !== targetDefaultCorner
  );
}
...
function createLayoutMap(
  layouts: OrbitLabelLayout[],
): Map<string, OrbitLabelLayout> {
  return new Map(
    layouts.map((layout) => [layout.id, cloneOrbitLabelLayout(layout)]),
  );
}

function layoutOrbitReferencePoint(layout: OrbitLabelLayout): {
  x: number;
  y: number;
} {
...
export function computeSweepDirectionFromSnapshots(
  ...
  const selectedToId = [...toLayoutById.values()].find(
...
function computeSweepDirectionFromRingRotations(
  fromRingRotationDeg: number,
  toRingRotationDeg: number,
): number {
...
export function interpolateOrbitTransitionPlan(
  ...
  const layout = plan.order
    .map((id) => {
      ...
      const layoutSweepDirection = resolveLayoutSweepDirection(
        source,
        target,
        centerX,
        centerY,
      ...
      const sourceSpokeAttached = isPointWithinLayoutRect(
        source,
        source.spokeEndX,
        source.spokeEndY,
      );
      const targetSpokeAttached = isPointWithinLayoutRect(
        target,
        target.spokeEndX,
        target.spokeEndY,
      );
      ...
      return {
        ...
        spokePath: buildSpokePath(
          frameAnchor.x,
          frameAnchor.y,
          frameSpokeEnd.x,
          frameSpokeEnd.y,
        ),
        branchPath: buildBranchPath(
          frameAnchor.x,
          frameAnchor.y,
          frameSpokeEnd.x,
          frameSpokeEnd.y,
  ...
  return {
    wheelRotationDeg: lerpNumber(
      plan.fromRingRotationDeg,
      plan.toRingRotationDeg,
      t,
    ),
    layout,
  };
  ...
  const previous =
    nextIndex === 0
      ? {
          ...
          minute: points[points.length - 1]!.minute - 1440,
        }
      : points[nextIndex - 1]!;
...
export function formatRelativeOffsetDirectionLabel(
  deltaMinutes: number,
): string {
  if (deltaMinutes === 0) {
    return "(same offset)";
...
function formatShortDateTime24InZone(
  timeZone: string | undefined,
  atMs: number,
): string {
  return new Intl.DateTimeFormat("en-US", {
...
function formatClockTimeFromSeconds(totalSeconds: number): string {
  const normalized =
    ((Math.round(totalSeconds) % SECONDS_PER_DAY) + SECONDS_PER_DAY) %
    SECONDS_PER_DAY;
...
function shortestCircularDelta(
  fromValue: number,
  toValue: number,
  modulus: number,
): number {
  ...
  return (
    ((((rawDelta + modulus / 2) % modulus) + modulus) % modulus) - modulus / 2
  );
}
...
function createCircularTrend(
  modulus: number,
): (oldValue: number, value: number) => number {
  return (oldValue: number, value: number) => {
    const normalizedOld =
      ((Math.round(oldValue) % modulus) + modulus) % modulus;
...
function getMinutesOfDayInZone(
  timeZone: string | undefined,
  atMs: number,
): number | null {
  try {
...
function shortestOffsetDeltaMinutes(
...
function cloneConicGradientStops(
  stops: ConicGradientStop[],
): ConicGradientStop[] {
...
function buildConicGradientStops(stops: SkyStop[]): ConicGradientStop[] {
  ...
  const mapped = stops
    ...
    .map((stop) => ({
      ...
      colorHex: stop.colorHex || "#081521",
      alpha: DAYLIGHT_SKY_STOP_NAMES.has(stop.name.trim().toLowerCase())
        ? 1
        : 0,
    }));
...
function areConicGradientStopsEqual(
  left: ConicGradientStop[],
  right: ConicGradientStop[],
): boolean {
...
function sampleNormalizedConicGradientStopAtMinute(
  ...
  if (nextIndex === -1) {
    ...
    const next = {
      ...normalizedStops[0]!,
      minute: normalizedStops[0]!.minute + 1440,
    };
    const span = next.minute - previous.minute;
    if (span <= 0) {
      return {
...
function interpolateConicGradientStops(
  ...
  return minutes.map((minute) => {
    const fromSample = sampleNormalizedConicGradientStopAtMinute(
      fromNormalized,
      minute,
    );
    const toSample = sampleNormalizedConicGradientStopAtMinute(
      toNormalized,
      minute,
    );
    const localProgress = useDaylightFanDelay
      ? fanOutProgressFromNoon(t, minute, fanOriginMinute)
      : t;
    return {
      minute,
      colorHex: mixHexColors(
        fromSample.colorHex,
        toSample.colorHex,
        localProgress,
      ),
...
function buildConicGradient(stops: ConicGradientStop[]): string {
  ...
  const segments = normalizedStops.map((stop) => {
    ...
    const color =
      stop.alpha < 0.999
        ? colorHexWithAlpha(stop.colorHex, stop.alpha)
        : stop.colorHex;
    return `${color} ${pct}%`;
  });
...
function locationDisplayLabel(location: PersistedLocationApiResult): string {
  if ((location.kind ?? "location") === "entity") {
    return (
      location.entityName?.trim() || location.nickname?.trim() || location.name
    );
  }
...
function sortByLabel(
  left: PersistedLocationApiResult,
  right: PersistedLocationApiResult,
): number {
  return locationDisplayLabel(left)
    .toLocaleLowerCase()
    .localeCompare(locationDisplayLabel(right).toLocaleLowerCase());
}
...
const LABEL_EMOJI_FAMILY =
  '"Apple Color Emoji", "Segoe UI Emoji", "Noto Color Emoji", sans-serif';
const BASE_REM_PX = 16;

function measureGroupedLabelWidth(
  group: OrbitLabelGroup,
  isMobile: boolean,
): number {
  ...
  const widestEntityRow = group.members.reduce((currentMax, member) => {
    ...
    const rowWidth =
      rowPadLeftPx + effectiveIconWidth + rowGapPx + nameWidth + rowPadRightPx;
    return Math.max(currentMax, rowWidth);
  }, 0);
  const footerDateTime = group.shortDateTime24 ?? group.time;
  const footerRelativeDelta = formatDecimalOffsetHours(
    group.relativeOffsetMinutes,
  );
  const footerOffsetLabel = group.isSelected ? "Now" : footerRelativeDelta;
  const footerText = `${footerDateTime} · ${footerOffsetLabel}`;
  const metaWidth =
    metaPadLeftPx + measureTextWidth(footerText, metaFont) + metaPadRightPx;

  return Math.ceil(
    chipPadLeftPx +
      Math.max(widestEntityRow, metaWidth) +
      chipPadRightPx +
      safetyBufferPx,
  );
}

export function useHomeClockModel(
  options: HomeClockModelOptions = {},
): HomeClockViewModel {
  const [savedLocations, setSavedLocations] = useState<
    PersistedLocationApiResult[]
  >([]);
  const [hasLoadedSavedLocations, setHasLoadedSavedLocations] =
  ...
  const [displayedNightMaskGradient, setDisplayedNightMaskGradient] =
    useState<string>(() =>
      buildNightMaskGradient(DEFAULT_CONIC_GRADIENT_STOPS),
    );
  const [areHourTicksVisible, setAreHourTicksVisible] = useState<boolean>(() =>
    prefersReducedMotion(),
  );
  const [gradientTransitionToken, setGradientTransitionToken] =
    useState<number>(0);
  const [isRingTransitioning, setIsRingTransitioning] =
    useState<boolean>(false);
  ...
  const [centerCopyTransitionKey, setCenterCopyTransitionKey] =
    useState<number>(0);
  const [displayOrbitLabelLayout, setDisplayOrbitLabelLayout] = useState<
    OrbitLabelLayout[]
  >([]);
  ...
  const visibleSavedLocations = useMemo(() => {
    if (
      !options.visibleLocationIds ||
      options.visibleLocationIds.length === 0
    ) {
      return savedLocations;
    }

    const visibleSet = new Set(options.visibleLocationIds);
    return savedLocations.filter((location) => visibleSet.has(location.id));
  }, [savedLocations, options.visibleLocationIds]);

  const sortedLocations = useMemo(() => {
    return [...visibleSavedLocations].sort((left, right) => {
      const leftOffset = left.timezone
        ? getTimeZoneOffsetMinutes(left.timezone, clockNowMs)
        : null;
      const rightOffset = right.timezone
        ? getTimeZoneOffsetMinutes(right.timezone, clockNowMs)
        : null;
      ...
      if (
        ...
        return (
          Math.abs(leftOffset - localOffsetMinutes) -
          Math.abs(rightOffset - localOffsetMinutes)
        );
      }

      return sortByLabel(left, right);
    });
  }, [visibleSavedLocations, clockNowMs, localOffsetMinutes]);
  ...
  const selectedLocation = useMemo(() => {
    ...
    return (
      visibleSavedLocations.find((location) => location.id === selectedId) ??
      null
    );
  }, [visibleSavedLocations, selectedId]);

  const selectedTimezone = selectedLocation?.timezone ?? browserTimezone;
  const selectedOffset = selectedTimezone
    ? getTimeZoneOffsetMinutes(selectedTimezone, clockNowMs)
    : localOffsetMinutes;
  ...
  const selectedCopyLabel = selectedLocation
    ? locationDisplayLabel(selectedLocation)
    : "your location";
  const centerTime = formatTimeInZone(selectedTimezone, clockNowMs);
  const [displayCenterTime, setDisplayCenterTime] =
    useState<string>(centerTime);
  const centerTimeParts = useMemo(
    () => parseClockTimeParts(displayCenterTime),
    [displayCenterTime],
  );
  const centerUtcOffsetParts = useMemo(
    () => splitUtcOffsetParts(safeSelectedOffset),
    [safeSelectedOffset],
  );
  const previousCenterTimePartsRef = useRef<ClockTimeParts | null>(
    centerTimeParts,
  );
  const previousCenterUtcOffsetPartsRef =
    useRef<UtcOffsetParts>(centerUtcOffsetParts);
  ...
  const shouldAnimateUtcHours =
    centerUtcOffsetParts.hours !==
    previousCenterUtcOffsetPartsRef.current.hours;
  const shouldAnimateUtcMinutes =
    centerUtcOffsetParts.minutes !==
    previousCenterUtcOffsetPartsRef.current.minutes;
  ...
  const wheelGradientStops = useMemo(
    () => buildConicGradientStops(sky?.stops ?? []),
    [sky],
  );
  const gradientFanMinute = useMemo(() => {
    const solarNoonStop = sky?.stops.find(
      (stop) =>
        stop.name === "solar_noon" && Number.isFinite(stop.minutesOfDay),
    );
    if (!solarNoonStop) {
  ...
  useLayoutEffect(() => {
    ...
    const ringDeltaDeg =
      previousBase === null
        ? 0
        : shortestAngleDeltaDeg(previousBase, sky.rotationDeg);
    const nextBase =
      previousBase === null ? sky.rotationDeg : previousBase + ringDeltaDeg;
  ...
  const orbitLabels = useMemo(() => {
    return sortedLocations
      .map((location): OrbitLabel | null => {
        ...
        const deltaMinutes = shortestOffsetDeltaMinutes(
          safeSelectedOffset,
          offset,
        );
        ...
        const minutesOfDay =
          getMinutesOfDayInZone(location.timezone, clockNowMs) ??
          normalizeMinutesOfDay(Math.round(clockNowMs / 60_000));
        ...
        return {
          ...
          shortDateTime24: formatShortDateTime24InZone(
            location.timezone,
            clockNowMs,
          ),
    ...
    return [...groups.entries()]
      .map(([timezoneKey, labels]): OrbitLabelGroup | null => {
        ...
        return {
          ...
          timezoneMeta:
            offsetMinutes === null
              ? primary.timezoneMeta
              : formatFloatingCardHeading(offsetMinutes),
  ...
  useLayoutEffect(() => {
    ...
    const selectionChanged =
      previousSelectionId !== null && selectedId !== previousSelectionId;
    ...
    const anchorSettled =
      targetAnchor === null ||
      Math.abs(targetAnchor - selectedOrbitAnchorDeg) < 0.001;
    if (!anchorSettled) {
      return;
    }
    ...
    const sourceLayout =
      displayOrbitLabelLayoutRef.current.length > 0
        ? displayOrbitLabelLayoutRef.current
        : orbitLabelLayout;
    const toLayouts = orbitLabelLayout.map((layout) =>
      cloneOrbitLabelLayout(layout),
    );
    const fromRing = Number.isFinite(displayWheelRotationRef.current)
      ? displayWheelRotationRef.current
      : wheelRotation;
    ...
    const tick = (frameNowMs: number) => {
      ...
      const progress = clamp(
        (frameNowMs - activePlan.startedAtMs) / activePlan.durationMs,
        0,
        1,
      );
  ...
  useEffect(() => {
    ...
    const shouldAnimateFromDefaultMidnight =
      ...
      !areConicGradientStopsEqual(
        wheelGradientStops,
        DEFAULT_CONIC_GRADIENT_STOPS,
      );
    const shouldAnimate =
      (gradientTransitionToken > lastAnimatedGradientTokenRef.current ||
        shouldAnimateFromDefaultMidnight) &&
      currentStops.length > 0 &&
      !prefersReducedMotion();
    if (!shouldAnimate) {
      const normalizedTarget = normalizeConicGradientStops(wheelGradientStops);
      displayedWheelGradientStopsRef.current =
        cloneConicGradientStops(normalizedTarget);
    ...
    const tick = (frameNowMs: number) => {
      const progress = clamp(
        (frameNowMs - startedAtMs) / GRADIENT_TRANSITION_MS,
        0,
        1,
      );
  ...
  const loadSavedLocations = useCallback(async () => {
    try {
      ...
      const payload = (await response.json()) as
        | PersistedLocationsApiResponse
        | ApiErrorResponse;
      if (!response.ok) {
        throw new Error(
          (payload as ApiErrorResponse).error?.message ??
            "Unable to fetch saved locations.",
        );
      }
  ...
  useEffect(() => {
    ...
    const selectionChanged =
      previousSelectedId !== null && selectedId !== previousSelectedId;
    lastSelectionForTimeCounterRef.current = selectedId;
    ...
    const minuteDelta = shouldAnimateMinutes
      ? shortestCircularDelta(startParts.minute, targetMinute, 60)
      : 0;
    ...
    const durationMs = Math.min(
      ...
      Math.max(
        500,
        Math.max(Math.abs(hourDelta) * 170, Math.abs(minuteDelta) * 22),
      ),
    );

    const tick = (frameNowMs: number) => {
      const progress = clamp((frameNowMs - startedAtMs) / durationMs, 0, 1);
      const nextHour = normalizeCircularValue(
        startParts.hour + Math.round(hourDelta * progress),
        24,
      );
      const nextMinute = shouldAnimateMinutes
        ? normalizeCircularValue(
            startParts.minute + Math.round(minuteDelta * progress),
            60,
          )
        : targetMinute;
      const nextTime = formatClockTimeFromSeconds(
        nextHour * 3600 + nextMinute * 60 + targetSecond,
      );
  ...
  useEffect(() => {
    if (
      selectedId &&
      visibleSavedLocations.some((location) => location.id === selectedId)
    ) {
      return;
    }
  ...
  }, [selectedId, visibleSavedLocations, sortedLocations]);
  ...
  useEffect(() => {
    ...
    const fetchSky = async ({
      atMs,
      morphOnResolve = false,
    }: { atMs?: number; morphOnResolve?: boolean } = {}) => {
      try {
  ...
  return {
    ...
    orbitLabelGroups,
    orbitLabelLayout:
      displayOrbitLabelLayout.length > 0
        ? displayOrbitLabelLayout
        : orbitLabelLayout,
    ringDiameter,
    labelOrbitSizePx,
