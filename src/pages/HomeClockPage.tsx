...
import type { SkyEnvironment as SharedSkyEnvironment, SkySecondOrderFactors as SharedSkySecondOrderFactors } from "../lib/time-in-place/types";
...
export function HomeClockPage() {
  ...
  const centerTimezoneMeta = formatTimezoneMeta(selectedTimezone, clockNowMs);
  ...
  useLayoutEffect(() => {
    ...
    const nextBase = previousBase === null
      ? sky.rotationDeg
      : previousBase + shortestAngleDeltaDeg(previousBase, sky.rotationDeg);
  ...
  const orbitLabels = useMemo(() => {
    const base = sortedLocations
      .map(location => {
        ...
        const minutesOfDay = getMinutesOfDayInZone(location.timezone, clockNowMs) ?? normalizeMinutesOfDay(Math.round(clockNowMs / 60_000));
        ...
        return {
          ...
          angleDeg: toAngleFromTopDeg(deltaMinutes),
  ...
  }, [sortedLocations, clockNowMs, safeSelectedOffset, selectedId, localOffsetMinutes, sky, isMobile, conceptTokens.laneStepPx]);
  ...
  useEffect(() => {
    ...
    const shouldAnimate = gradientTransitionToken > lastAnimatedGradientTokenRef.current && displayedWheelGradient.length > 0;
    if (!shouldAnimate) {
  ...
  return (
    <main className="home-shell home-shell--zenith">
      <div className="home-viewport-sky" aria-hidden="true">
        <div
          className={`home-viewport-sky-layer is-current ${isRingTransitioning ? "is-switching" : ""}`}
          style={{
            backgroundImage: displayedWheelGradient,
            transform: `rotate(${wheelRotation}deg)`,
          }}
        />
        {previousWheelGradient ? (
          <div
            className={`home-viewport-sky-layer is-previous ${isRingTransitioning ? "is-switching" : ""} ${
              isGradientTransitioning ? "is-fading-out" : ""
            }`}
            style={{
              backgroundImage: previousWheelGradient,
              transform: `rotate(${wheelRotation}deg)`,
            }}
          />
        ) : null}
      ...
      <section className="home-ring-stage" aria-label="Sky ring 24 hour view">
        <div className="home-ring-frame" ref={ringFrameRef} style={conceptVars}>
          <div className={`home-sky-ring ${isRingTransitioning ? "is-switching" : ""}`} style={{ transform: `rotate(${wheelRotation}deg)` }}>
            {previousWheelGradient ? (
              <div
            ...
            <div className="home-sky-ring-layer is-current" style={{ backgroundImage: displayedWheelGradient }} aria-hidden="true" />
          </div>
          ...
          <div className="home-center-readout">
            <div key={centerCopyTransitionKey} className="home-center-copy">
              ...
              <p className="home-center-time">{centerTime}</p>
              <p className="home-center-meta">{centerTimezoneMeta}</p>
            </div>
            {ringError ? (
          ...
          <div className="home-label-orbit" role="listbox" aria-label="Saved locations by 24 hour offset">
            {orbitLabels.map(label => {
              const baseRadius = ringDiameter > 0 ? ringDiameter * 0.42 : 180;
              ...
              return (
                <button
                  ...
                  className={`home-orbit-label ${label.isSelected ? "is-selected" : ""} ${label.isLocal ? "is-local" : ""}`}
                  onClick={() => setSelectedId(label.id)}
                  style={{
                    transform: `translate(-50%, -50%) rotate(${label.angleDeg}deg) translateY(-${radiusPx}px) rotate(${-label.angleDeg}deg)`,
                    zIndex: label.isSelected ? 18 : 8,
                  }}
                  title={`${label.label} · ${label.time} · ${label.timezoneMeta} · ${label.relativeLabel}`}
                >
                  <span className="home-orbit-chip" style={{ "--orbit-accent": label.skyColorHex } as React.CSSProperties}>
